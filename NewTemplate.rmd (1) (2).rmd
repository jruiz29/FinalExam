...
### Graphical Descriptive Results
    ...
    First, we will look at a barchart to answer. As well as a boxplot.
...
* The Chi- Square test shows that we would reject the null hypothesis based on the result of degrees of freedom. When looking at the chi-square statistic equalling 29.155 we see that it is not the expected outcome. We see the jab was the only variable close to the expected outcome as we can see originally out of the 60 scorpions being tested only 6 jabbed in the control group while some scorpions in threat gave 5 jabs. Looking at those that gave a dry and wet sting we can see they were the only two variables that contributed making a large differnece.


    Another Inferential test we could look at would be a t.test to answer.
...
### Graphical Descriptive Results
    ...
    First, we will look at a barchart to answer. Then will consider comparing row percent.
...
```{r}
chisqtestGC(sze,simulate.p.value = TRUE, graph=TRUE)
```

When looking at who contributed the most, dry sting and no sting gave us an average of 15.Looking at the degrees of freedom we see that is 3 rows which are contributing factors.
...
* The results of our inferential test say the probability of getting results like this observed is 0.0004256, if we believe the null hypothesis. So this convincing evidence leads us to reject the null hypothesis. Concludes that the reaction time control of a scorpion shows a significant difference when threatened by a predator.
